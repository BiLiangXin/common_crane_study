...
.
├── data/
│   ├── Common_Crane_Dataset2.csv       # Complete  tracking dataset used in the study (>17,000  fixes)
...
│   └── README.md
├── st-dbscan/
│   ├── run_pipeline.py                 # Main program for  cleaning, nighttime detection, and T-DBSCAN
...
`data/Common_Crane_Dataset2.csv` contains  tracking data for Common Cranes.
...
| `Available`  | Indicates whether the BDS record is valid; by default, records with the value `Available` are retained |
...
python pipeline.py `
  --points "..\st-dbscan\outputs_10000.0\BDS_points_with_cluster_10000.0.csv" `
...
### BDS Processing and T-DBSCAN

- Parse timestamps and remove invalid BDS fixes, invalid coordinates, and records with unparseable timestamps.
...
- Construct continuous flight lines from non-habitat BDS points.
...
| File                            | Description                                                  |
| ------------------------------- | ------------------------------------------------------------ |
| `BDS_points_with_cluster_*.csv` | Cleaning results, nighttime attributes, cluster IDs, and event types for each BDS point |
| `flight_points_*.csv`           | BDS points classified as non-habitat flight points for subsequent corridor analysis |
...
| `removed_invalid_points.csv`    | Original BDS points identified as invalid                    |
| `removed_speed_outliers.csv`    | BDS points removed because of unrealistic movement speeds    |
...
- The BDS dataset contains spatial locations of wild animals. Before publishing or redistributing the complete dataset, comply with the licensing, privacy, and data-sharing requirements of the data provider.
